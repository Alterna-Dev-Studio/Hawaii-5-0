# Hawaii5O  [![Nuget](https://img.shields.io/nuget/v/Hawaii5O.svg?colorB=green)](https://www.nuget.org/packages/Hawaii5O)
...
- **Bug fixes** -- null-ref guards, identifier sanitization, and more (see [CHANGELOG.md](CHANGELOG.md))

### Looking for a Fable-first client?

If you're building a Fable/SolidJS frontend and want a client optimized purely for browser use, check out [Kona](https://github.com/g5becks/Kona) -- another Hawaii fork that focuses exclusively on Fable with `promise {}` computation expressions, Thoth.Fetch integration, and native `JS.Promise` support. Hawaii5O generates both dotnet and Fable clients; Kona goes all-in on the Fable side.

### Clanker Driven Development

This project includes code generated by clankers (Claude). All clanker-generated contributions are attributed in commit messages via `Co-Authored-By` tags.

### Features
 - Supports any OpenApi/Swagger schema in form of JSON or YAML, compatible with those that can be read by [OpenAPI.NET](https://github.com/microsoft/OpenAPI.NET) (YAML support based on work by [@g5becks](https://github.com/g5becks) in [Kona](https://github.com/g5becks/Kona))
...
```
dotnet tool install -g Hawaii5O
```
...
```
YAML schemas are also supported — just point to a `.yaml` or `.yml` file:
```json
{
...
After you have the configuration ready, run `hawaii5o` in the directory where you have the `hawaii.json` file:
```
hawaii5o
```
You can also tell hawaii5o where to find the configuration file if it wasn't named `hawaii.json`, for example
```
hawaii5o --config ./petstore-hawaii.json
```
...
```
hawaii5o --from-odata-schema {schema} --output {output}
```
where
...
```
hawaii5o --from-odata-schema "https://services.odata.org/V4/(S(s3lb035ptje4a1j0bvkmqqa0))/TripPinServiceRW/$metadata" --output ./TripPin.json
```
### Sample Applications
...
### Version
You can ask hawaii5o which version it is currently on:
```
hawaii5o --version
```
...
```
hawaii5o --no-logo
hawaii5o --config ./hawaii.json --no-logo
```
...
Hawaii-5-0 stands on the shoulders of [Zaid Ajaj](https://github.com/Zaid-Ajaj), who created Hawaii and built it into a fantastic tool through dozens of releases. We also owe a huge thank you to the community contributors whose work -- both merged upstream and unmerged PRs -- made this fork possible:

- **[Richard Webb](https://github.com/Numpsy)** -- guid format support, DateTimeOffset query strings, EscapeDataString, async builder fix, text/plain requestBody support (upstream PRs #2, #7, #8, #14; unmerged PR #65 incorporated here)
- **[Michael May](https://github.com/MichaelMay81)** -- uuid/guid response format, path-level parameters, request headers as request parts (upstream PRs #33, #34, #36)
- **[Franco Tiveron](https://github.com/francotiveron)** -- CancellationToken support, nullable Dictionary value handling (upstream PRs #45, #47)
- **[Viktor Schepik](https://github.com/vschep)** -- unlimited response status codes (upstream PR #50)
- **[varon](https://github.com/varon)** -- multipart form Content-Disposition fix, banner/build fix, project name generation fix (upstream PRs #53, #54, #56)
- **[Christoph Winter](https://github.com/christoph-feb22)** -- status code 422 support (upstream PR #49)
- **[Roland Andrag](https://github.com/AgricoZA)** -- typo fixes (upstream PR #16)
- **[Houston Haynes](https://github.com/houstonhaynes) / [SpeakEZ Technologies](https://github.com/speakeztech)** -- null-ref guards, identifier sanitization for Cloudflare spec, .NET 10 + FSharp.SystemTextJson upgrade (unmerged upstream PR #57; fork contributions incorporated here)
