namespace UntypedObjectArrayProperty.Http
...
module Serializer =
    ...
    /// Format() result (as generated by Hawaii) and the case name, both
    /// compared case-insensitively and with hyphens/underscores ignored.
    type TolerantStringEnumConverter() =
        ...
        override this.CreateConverter(typeToConvert: System.Type, _options: JsonSerializerOptions) =
            let converterType = typedefof<TolerantStringEnumInnerConverter<_>>.MakeGenericType(typeToConvert)
            System.Activator.CreateInstance(converterType) :?> JsonConverter

    and TolerantStringEnumInnerConverter<'T>() =
        inherit JsonConverter<'T>()
        let normalize (x: string) = x.Replace("_", "").Replace("-", "").ToLowerInvariant()
        ...
        override _.Read(reader: byref<Utf8JsonReader>, _typeToConvert: System.Type, _options: JsonSerializerOptions) : 'T =
            if reader.TokenType = JsonTokenType.Null then Unchecked.defaultof<'T>
            else
                let s = reader.GetString()
                let s = if isNull s then "" else s
                let target = normalize s
                let tryMatch (uc: UnionCaseInfo) =
                    if uc.GetFields().Length = 0 then
                        let inst = FSharpValue.MakeUnion(uc, [||]) :?> 'T
                        let fmt =
                            match objectType.GetMethod("Format") with
                            | null -> uc.Name
                            | m ->
                                match m.Invoke(inst :> obj, [||]) with
                                | :? string as str -> str
                                | _ -> uc.Name
                        if normalize fmt = target || normalize uc.Name = target then Some inst else None
                    else None
                match cases |> Array.tryPick tryMatch with
                | Some v -> v
                | None -> raise (JsonException(sprintf "Cannot map '%s' to %s" s objectType.FullName))

        override _.Write(writer: Utf8JsonWriter, value: 'T, _options: JsonSerializerOptions) =
            if isNull (value :> obj) then writer.WriteNullValue()
            else
                let t = value.GetType()
                match t.GetMethod("Format") with
                | null ->
                    let (uc, _) = FSharpValue.GetUnionFields(value :> obj, t)
                    writer.WriteStringValue(uc.Name)
                | m ->
                    match m.Invoke(value :> obj, [||]) with
                    | :? string as sv -> writer.WriteStringValue(sv)
                    | _ ->
                        let (uc, _) = FSharpValue.GetUnionFields(value :> obj, t)
                        writer.WriteStringValue(uc.Name)
