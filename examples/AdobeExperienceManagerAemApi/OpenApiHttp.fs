namespace AdobeExperienceManagerAemApi.Http
    ...
    type TolerantStringEnumConverter() =
        ...
        override _.ReadJson(reader: JsonReader, objectType: System.Type, existingValue: obj, serializer: JsonSerializer) : obj =
            if reader.TokenType = JsonToken.Null then null
                ...
                let s = match reader.Value with null -> "" | v -> v.ToString()
                ...
                let tryMatch (uc: UnionCaseInfo) =
                    if uc.GetFields().Length = 0 then
                        let inst = FSharpValue.MakeUnion(uc, [||])
                        let fmt =
                            match inst.GetType().GetMethod("Format") with
                            | null -> uc.Name
                            | m ->
                                match m.Invoke(inst, [||]) with
                                | :? string as str -> str
                                | _ -> uc.Name
                ...
                | Some v -> v
                | None -> raise (JsonSerializationException(sprintf "Cannot map '%s' to %s" s objectType.FullName))
        ...
        override _.WriteJson(writer: JsonWriter, value: obj, serializer: JsonSerializer) =
            if isNull value then writer.WriteNull()
                ...
                | null ->
                    let (uc, _) = FSharpValue.GetUnionFields(value, t)
                    writer.WriteValue(uc.Name)
                | m ->
                    match m.Invoke(value, [||]) with
                    | :? string as sv -> writer.WriteValue(sv)
                    | _ ->
                        let (uc, _) = FSharpValue.GetUnionFields(value, t)
                        writer.WriteValue(uc.Name)
